...
describe('Producción documental externa sin archivo - Por flujo con revisor', () => {
  let radicadoE;
  let radicadoS;
  before(() => {
  ...
  it('Flujo número 28', { defaultCommandTimeout: 40000 }, () => {
    ...
    cy.iframe().find('p-header i').click();
    cy.iframe().find('.ng-trigger-overlayAnimation li').contains("Trazabilidad").click();
    cy.iframe().find('.ui-scrollpanel-wrapper .StepProgress').first().should('include.text', 'Revisar Documento').click();
    cy.get('a .letrasMinagricultura').click();
    cy.get('.ultima-menu li[role="menuitem"]').contains('Cerrar').click();
